...
const TableDataCell = ({ attributes, children, element, blockId }: PluginElementRenderProps) => {
  ...
  const parentPath = Elements.getElementPath(editor, blockId, element);
  if (Array.isArray(parentPath) && parentPath.at(-1) === 0) {
    const parentElement = Elements.getElement(editor, blockId, { path: parentPath, type: 'tr' });
  }
