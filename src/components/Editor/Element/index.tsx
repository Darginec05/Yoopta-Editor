...
const Element: FC<RenderElementProps | any> = ({ attributes, children, element }) => {
  ...
  switch (element.type) {
    case 'block-quote':
      return (
        <blockquote onMouseEnter={() => onMouseEnter('block-quote')} className={s.blockquote} {...attributes}>
          {children}
        </blockquote>
      );
    case 'bulleted-list':
    ...
    case 'heading-one':
      return (
        <h1 onMouseEnter={() => onMouseEnter('heading-one')} className={s['heading-one']} {...attributes}>
          {children}
        </h1>
      );
    case 'heading-two':
      return (
        <h2 onMouseEnter={() => onMouseEnter('heading-two')} className={s['heading-two']} {...attributes}>
          {children}
        </h2>
      );
    case 'heading-three':
      return (
        <h3 onMouseEnter={() => onMouseEnter('heading-three')} className={s['heading-three']} {...attributes}>
          {children}
        </h3>
      );
    case 'list-item':
